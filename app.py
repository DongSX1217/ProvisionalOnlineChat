...
class Message:
    ...
    def delete():
        ...
        try:
            ...
            operator_location = API.get_ip_location(user_ip)
                ...
                for i, msg in enumerate(chat_history):
                    if msg['sort_key'] == message_id:
                        ...
                        Log.operation(
                            ...
                            target_ip=msg['ip'],
                            target_location=API.get_ip_location(msg['ip']),
                            content=msg['message']
                        )
                        return jsonify({'status': 'success'})                       
                for i, h in enumerate(highlights):
                    if h['sort_key'] == message_id:
                        ...
                        Log.operation(
                            ...
                            operator_location=operator_location,
                            target_ip=h['ip'],
                            target_location=API.get_ip_location(h['ip']),
                            content=h['message']
                        )
                        return jsonify({'status': 'success'})
    ...
    @app.route('/message/toggle_highlight', methods=['POST'])
